...
router.route('/login').post(authController.validateAdminAccount, authController.loginAccount);
router
  .route('/signup')
  .post(validateAdminDetails(false), checkValidationErrors, adminController.signupAdmin);
router
  .route('/refreshToken')
...
router
  .route('/updateMe')
  .patch(validateAdminDetails(true), checkValidationErrors, adminController.updateMe);
router
  .route('/uploadProfilePicture')
  .post(adminController.uploadFile, adminController.resizeImage, adminController.uploadPhotoToS3);
router.route('/profilePicture/:filename').get(adminController.streamProfilePicture);
